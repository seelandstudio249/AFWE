...
namespace Assets.Scripts {
	...
	public class PositionCalculator {
		...
		public static Vector3? CalculatePointInSpace(Vector2 yoloItem, CameraTransform cameraTransform) {
			//Vector2 positionInImage = ScaleBack(new Vector2((yoloItem.xmin + yoloItem.xmax) / 2, (yoloItem.ymin + yoloItem.ymax) / 2));
			Vector2 positionInImage = ScaleBack(yoloItem);
		...
		//public static Vector3[] CalculateCornerPoints(SignalRNotificationMessage yoloItem, CameraTransform cameraTransform) {
		...
		//public static Vector3 GetPositionInSpace(SignalRNotificationMessage notification, Vector2 positionInImage) {
		...
		//	float aspectRatioCorrection = (float)Parameters.ModelImageResolution.x / Parameters.ModelImageResolution.y;

		//	return castOrigin + Parameters.HeightOffset * Vector3.up + castDirection +
		//		   Vector3.right * (positionInImage.x * Parameters.VirtualProjectionPlane.x * aspectRatioCorrection) -
		...
		private static Vector3? CastOnSpatialMap(Vector3 positionInSpace, CameraTransform cameraTransform) {
			...
			if (PhysicsCaller.SphereCastOnSpatialMesh(sphereCastOrigin, direction, out RaycastHit hitInfo)) {
				...
				float distance = Vector3.Distance(sphereCastOrigin, hitInfo.point);

				if (distance > 8f) {
					Debug.Log("Hit object is beyond 8 meters.");
					return null;
				}
